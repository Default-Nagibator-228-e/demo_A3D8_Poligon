...
public class MaterialProcessor {
			...
			if (processedMesh != null) {
				...
				for (var surfaceInd:uint = 0; surfaceInd < meshSurfacesQuantity; surfaceInd++) {
					...
					if (!materialTextures[BUMP]) {
						//materialTextures[BUMP] = new BitmapTextureResource(DEFAULT_BUMP);
					}
	 ...
	 */
	public function setupExternalTexture(texture:ExternalTextureResource, source:Object, isATF:Boolean = false):void {
		if (source) {
	...
	public function setupExternalTextureFromBitmap(textureResource:ExternalTextureResource, resourceData:BitmapData):void {
		//size of bitmap must be power of 2
		resourceData = fitTextureToSizeLimits(resourceData);

		//create texture and upload bitmap data
		var texture:Texture = context.createTexture(resourceData.width, resourceData.height, Context3DTextureFormat.BGRA, true);
